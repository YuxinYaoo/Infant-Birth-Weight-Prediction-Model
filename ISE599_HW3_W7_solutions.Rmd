...
```{r}
bw = read.csv("birthweight_predict.csv")
...
**(ii) Convert numeric variables to factors where appropriate**.
...
**(i) Create a full regression model to predict baby birthweight using all the independent variables in the dataset. Compute the in-sample and out-of-sample R2 for your model.**
...
**(ii) Create a reduced model based on variance inflation factors and statistical significance results from the full model. Compute the in-sample and out-of-sample R2 for your model. **
...
- Calculate the time it takes to run the combination of cross validation and iterations
- Perform cross validation with lasso over iterations using the `cv.glmnet()` function from the `glmnet` package. This exact code has not been shown in lab.
